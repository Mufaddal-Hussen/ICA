...
//                conn = DriverManager.getConnection("jdbc:derby://localhost:1527/voting", "root", "root");
                ...
                ps = conn.prepareStatement("insert into best_actress(actress_name,user) values(?,?)");
                ps.setString(1, request.getParameter("actress_name") );
                ps.setString(2, request.getParameter("userid"));
              
               
                if(ps.executeUpdate()!=0){
                    response.sendRedirect("bestactress.jsp");
//                    out.print("inserted");
                }else{
                    response.sendRedirect("bestactress.jsp");
