 ...
 * @author Dell
 */
public class vote6 extends HttpServlet {
...
//                conn = DriverManager.getConnection("jdbc:derby://localhost:1527/voting", "root", "root");
                ...
                ps = conn.prepareStatement("insert into best_show(show_name,user) values(?,?)");
                ps.setString(1, request.getParameter("show_name") );
                ps.setString(2, request.getParameter("userid"));
              
               
                if(ps.executeUpdate()!=0){
                    response.sendRedirect("bestshow.jsp");
//                    out.print("inserted");
                }else{
                    response.sendRedirect("bestshow.jsp");
