...
//                conn = DriverManager.getConnection("jdbc:derby://localhost:1527/voting", "root", "root");
                ...
                ps = conn.prepareStatement("insert into best_actor(actor_name,user) values(?,?)");
                ps.setString(1, request.getParameter("actor_name") );
                ps.setString(2, request.getParameter("userid"));
              
               
                if(ps.executeUpdate()!=0){
                    response.sendRedirect("bestactor.jsp");
//                    out.print("inserted");
                }else{
                    response.sendRedirect("bestactor.jsp");
