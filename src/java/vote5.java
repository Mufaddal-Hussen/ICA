 ...
 * @author Dell
 */
public class vote5 extends HttpServlet {
...
//                conn = DriverManager.getConnection("jdbc:derby://localhost:1527/voting", "root", "root");
                ...
                ps = conn.prepareStatement("insert into best_singer(singer_name,user) values(?,?)");
                ps.setString(1, request.getParameter("singer_name") );
                ps.setString(2, request.getParameter("userid"));
              
               
                if(ps.executeUpdate()!=0){
                    response.sendRedirect("bestsinger.jsp");
//                    out.print("inserted");
                }else{
                    response.sendRedirect("bestsinger.jsp");
